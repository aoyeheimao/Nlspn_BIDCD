...
class RENDER(BaseDataset):
    ...
    def __init__(self, args, mode):
        super(RENDER, self).__init__(args, mode)
        if self.mode == 'train':
            self.dir = '..'  # dir_path
        ...
        # ])
        self.K = torch.Tensor([572.4114,
                               573.57043,
                               325.2611,
                               242.04899
                               ])
        # 照片大小 需要定义
        height, width = (480, 640)  # (720, 1280)
    ...
    def __getitem__(self, idx):
        ...
        dep = np.array(cv2.imread(self.gt_depth[idx], 2)) # 恢复的目标深度
        scan_dep = np.array(cv2.imread(self.raw_dept[idx], 2)) #工件扫描深度图
...
#                     default='NYU',
#                     # default='KITTIDC',
#                     choices=('NYU', 'KITTIDC', 'RGBDDATASET','DFX802', 'RENDER'),
#                     help='dataset name')
...
#                     type=str,
#                     default='scan',
#                     choices=('scan', 'generate', 'scan+generate'),
...
# print('\n')
#
# args_main.depth_type = 'scan+generate'
#
# for i in range(40,60):
#     dataset = RENDER(args_main,'train')[i]
#     # dataset = RENDER(args_main,'test')[i]
#     print(dataset['rgb'].shape)
#     print('gt_max:', dataset['gt'].max(), 'gt_min:', dataset['gt'].min(), 'mean:', dataset['gt'].mean())

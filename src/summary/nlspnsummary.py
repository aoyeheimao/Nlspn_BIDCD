    ...
    def save(self, epoch, idx, sample, output):
        with torch.no_grad():
            if self.args.save_result_only:
                ...
                np.savez(self.path_output + '/array_result_save.npz', rgb=rgb_tmp, dep=dep_tmp, pred=pred_tmp,
                         pred_gray=pred_gray_tmp, gt=gt_tmp, feat_init=feat_init_tmp)
